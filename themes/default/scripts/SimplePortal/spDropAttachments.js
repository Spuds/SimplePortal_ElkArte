 ...
 * while creating articles and editing them.
 */
	...
	const dragDropAttachment = (function(params) {
		...
		let allowedExtensions = [],
			...
			removeFileFromServer = function(options) {
				...
				postForm.append('articleid', articleid);
				postForm.append(elk_session_var, elk_session_id);

				fetch(elk_prepareScriptUrl(elk_scripturl) + 'action=portal;sa=rmattach;api=json;', {
					method: 'POST',
					headers: {
		...
		window.onload = function() {
			...
			document.querySelectorAll('.drop_area_fileselect_text').forEach(anchor => {
				anchor.addEventListener('click', function(e) {
					e.preventDefault();
					// Trigger the file dialog on the original input
					input.click();
				});
			});
